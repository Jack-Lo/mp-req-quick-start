...
OK，我们在项目根目录下新建一个`api`目录用来存放api的定义文件，然后新建`user.js`文件，并做如下定义：
...
这样，我们就实现了一个接口的定义，接下来需要将这个插件安装到req上，打开`app.js`，使用`req.use`方法来接入插件：
...
由于初始化的时候我们传了apiUrl，之后的api定义我们就可以统一使用apiUrl来拼装url了，我们修改一下原来的`api/user.js`：
...
以上，就是定义一个接口的全部过程，当然，只是在第一次定义一个类的时候过程麻烦些，后续有属于`user`类的接口，只需要在`api/user.js`文件中补充对应的方法就可以了。
